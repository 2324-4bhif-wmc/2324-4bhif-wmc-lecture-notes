...
A function both consumes and produces data.
...
The device is then consumed by the store.
...
It is a Kotlin function that is annotated with @Composable.
This annotation tells the Compose compiler that the functions is meant for UI construction.

These are the only functions which have a capital letter at the beginning of their name.
They are outside any class and are top-level functions.
