...
<.> Durch "Accept" wird nur diese Form an Daten angenommen.
Hier ist es nur normaler Text
...
Der Inhalt der Antworten kann dabei dynamisch, also im Moment der Anfrage, erstellt werden und muss nicht bereits statisch (etwa in Form einer HTML-Seite) für den Webserver verfügbar sein.Als Jakarta Servlet (früher Java Servlet) bezeichnet man Java-Klassen, deren Instanzen innerhalb eines Webservers Anfragen von Clients entgegennehmen und beantworten.
Der Inhalt der Antworten kann dabei dynamisch, also im Moment der Anfrage, erstellt werden und muss nicht bereits statisch (etwa in Form einer HTML-Seite) für den Webserver verfügbar sein. https://de.wikipedia.org/wiki/Jakarta_Servlet[wikipedia^]
...
* `stream`: Man bekommt Ergebnisse Stück für Stück und nicht auf einmal.
Somit kann man mit den bereits erhaltenen Daten gleich weiterarbeiten.
...
A domain object model is a class diagramm without any "technical" classes like "Repository", "Service", "Controller", "DAO", ...
...
image::symbolic-dom.png[Symbolic DOM]

image:dom-in-browser.png[DOM in a browser]

=== Functional Programming Paradigm

Functional programming is a declarative paradigm that emphasizes pure mathematical functions over imperative methods.
It prioritizes "what to solve" rather than "how to solve" and employs expressions instead of statements, with functions possessing unique characteristics.
...
=== Android Architecture (presented by Linus Nestler)

==== BehaviourSubject

BehaviorSubjects are a type of Subject that: Has an initial value.
Subscribers will receive the last emitted value upon subscription

==== Model

One central model which is used by the whole application ("single source of truth").
It contains the data of the application.

==== Store

It is responsible for managing the state of the application and for providing the data to the UI.

The MainView subscribes to the store and gets the data from it.

The store has a function which is called `next` and it is used to update the state of the application.

==== Producer

A producer is a class that is responsible for providing the data to the UI.
...
Functions both consume and produce data.
...
A consumer is a class that is responsible for consuming the data from the UI.

*Example:*

The user selects a device to connect to via Bluetooth.
The store then consumes the device.
...
You can find a tutorial https://developer.android.com/jetpack/compose/setup[here] and https://developer.android.com/jetpack/compose/tutorial[here].

The test project is also available in the lecture notes https://github.com/2324-4bhif-wmc/2324-4bhif-wmc-lecture-notes[repository]
where the kotlin build system was used.

==== Composable Functions

A composable function is a function that can be called by the UI framework to render the UI.
It is a Kotlin function annotated with `@Composable`.
This annotation tells the Compose compiler that the functions are meant for UI construction.

These are the only functions that have a capital letter at the beginning of their name.
They are outside any class and therefore are top-level functions.
...
https://medium.com/javarevisited/javas-records-vs-kotlin-s-data-classes-b3fef851155a[This] article explains the differences between Java's Records and Kotlin's Data Classes.

*tl;dr*: Kotlin's Data Classes are roughly equivalent to Java's Records.

=== Frontend

==== Material Design

https://m3.material.io/[Material Design] is a design language developed by Google. The website has many useful tips and tricks for designing a frontend.
